...
public class PubSubTopicList extends HttpServlet {
  /**
	 * 
	 */
	private static final long serialVersionUID = -7394260602938306597L;
	
	final Logger LOG = Logger.getLogger(PubSubTopicList.class.getName());
	...
	try{
     ...
     // define HTTP_TRANSPORT and JSON_FACTORY
        ...
        Pubsub.Projects.Topics.List listMethod =
                pubsub.projects().topics().list("projects/"+Constants.PROJECT_ID);
        ...
        do {
            ...
            if (topics != null) {
                for (Topic tp : topics) {
                		topicList.add(tp.getName());
                }
            }
            nextPageToken = response.getNextPageToken();
        } while (nextPageToken != null);
        
        String responseToRet = new Gson().toJson(topicList);
