...
public class PubSubTopicSubscribeList extends HttpServlet {
  /**
	 * 
	 */
	private static final long serialVersionUID = -7394260602938306597L;
	
	final Logger LOG = Logger.getLogger(PubSubTopicSubscribeList.class.getName());
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse resp)
	...
	try{
        ...
        Pubsub.Projects.Subscriptions.List listMethod = pubsub.projects()
                .subscriptions().list("projects/"+Constants.PROJECT_ID);
        ...
        do {
            ...
            if (subscriptions != null) {
                for (Subscription subscription : subscriptions) {
                	subscriptionList.add(subscription.getName());
                }
            }
            nextPageToken = response.getNextPageToken();
        } while (nextPageToken != null);
        
        String responseToRet = new Gson().toJson(subscriptionList);
